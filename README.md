...
왼쪽에서 장애 증상을 선택한 뒤 안내된 단계로 진행합니다. 단계 이동만으로 외부 요청을 실행하지 않습니다.

1. **무선 연결 · L1/L2:** 무선 진단 시작을 눌러 Wi-Fi와 AP, 신호·채널·PHY를 확인합니다.
2. **IP·실제 경로 · L3:** 기본 연결 설정 확인으로 IP·게이트웨이·DNS 설정 유무를 점검한 뒤 Wi-Fi·유선·VPN 동시 연결을 확인하고 필요한 경우 내부·프록시 대상의 OS 경로를 비교합니다.
3. **프록시·서비스 확인:** 프록시 설정과 대상별 경로를 확인합니다. HTTP·인증 결과는 실제 측정 후 해석합니다.
4. **성능 비교:** 승인된 내부·외부 대상을 측정합니다. 반복 측정과 브라우저 관찰은 보조 선택으로 제공합니다.
5. **결과·보고서:** 저장 전 요약에서 관찰 결과·미확인 항목·다음 행동을 확인하고 상세 근거를 보고서로 저장합니다.

기존 개별 기능은 **고급: 전체 도구 보기**에 있습니다. 유선 비교 순서와 해석 제한은 [무선 안내형 진단](docs/WIRELESS_GUIDED_DIAGNOSTICS.md)을 참고하십시오. OSI 전 계층의 정상 여부나 프록시 장애를 자동 확정하지 않습니다.
