...
```text
release-trigger/v0.1.0-alpha.5
...
```text
This branch exists only to dispatch the verified manual prerelease workflow for v0.1.0-alpha.5.
```
...
1. GitHub Release API에서 태그·prerelease 상태·자산 네 개를 확인합니다.
2. `SHA256SUMS.txt`와 실제로 내려받은 파일의 해시를 확인합니다.
3. Portable ZIP의 `BUILD_INFO.txt`가 태그 버전과 검증된 `main` SourceRevision을 가리키는지 확인합니다.
4. `release-trigger/v…` 브랜치를 삭제합니다.
5. 실제 Windows 11 회사 환경은 [`RELEASE_VALIDATION.md`](RELEASE_VALIDATION.md)에 따라 사용자가 검증합니다.

실제 릴리스 태그는 삭제하지 않습니다. Release API에서 공개 상태가 확인되기 전에는 새 버전을 공개 최신으로 보고하지 않습니다.
