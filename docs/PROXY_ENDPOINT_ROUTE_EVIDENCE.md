...
프록시 후보 2
  RouteStatus=Success
  SelectedInterfaceCategory=Ethernet
  WlanCorrelationStatus=DifferentInterface
```
...
- IP literal이면 DNS 생략
- Windows `GetBestInterfaceEx` 계열 로컬 라우팅 판정
- 로컬 `NetworkInterface` 정보 상관 분석
...
DNS 결과에서 최대 16개의 유효 IPv4·IPv6 주소를 기존 route reader가 확인합니다.

- 각 주소의 Windows 최적 인터페이스
- 주소별 성공·실패
- 하나의 프록시 호스트가 여러 인터페이스로 분기되는지
- 최종 단일 인터페이스 확정 가능 여부

DNS 결과가 여러 주소군으로 나뉘어 단일 인터페이스가 확정되지 않으면 `DestinationRouteEvidenceStatus.MultipleInterfaces` 또는 관련 부분 상태가 유지됩니다.
