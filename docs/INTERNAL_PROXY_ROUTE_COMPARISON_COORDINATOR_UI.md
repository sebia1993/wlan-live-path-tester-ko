...
기존 작업 상태 필드 이름을 안전하게 조회하는 방식은 임시 호환 계층입니다. 장기적으로는 모든 측정 기능이 공통 작업 상태 서비스를 사용하도록 정리해야 합니다.
...
비교 실행과 보고서 생성을 별도 상태로 관리합니다.

- 비교 중 보고서 생성 차단
- 보고서 생성 중 새 비교 차단
- 보고서 생성 중 `경로 확인 중지` 비활성화
- 보고서 완료 후 기존 탭과 입력 상태 복원

애플리케이션 버전은 `AssemblyName.Version`을 명시적으로 읽고 null 여부를 분기합니다. null 조건 연산자를 줄바꿈으로 분리하지 않아 Release 빌드 구문 오류를 방지합니다.
...
11. 경로 비교·보고서 생성 중 다른 탭이 잠기고 종료 후 이전 상태로 복원되는지 확인합니다.
