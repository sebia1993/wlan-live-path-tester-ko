...
internal static class Program
{
    private static int Main()
    {
        ...
        // Async injected readers must run after module initialization.
        try
        {
            WindowsRouteProxyImporterTests.RunAsync().GetAwaiter().GetResult();
            ProxyImportRawInputTests.RunAsync().WaitAsync(TimeSpan.FromSeconds(40)).GetAwaiter().GetResult();
        }
        ...
        if (!CheckProxySettingsBoundary()) return 1;
        if (!CheckNetworkEnvironmentBoundary()) return 1;
        if (!CheckWlanIdentityBoundary()) return 1;

        WlanReadResult stoppedService = new(
            WlanReadStatus.NativeError, Array.Empty<WlanSnapshot>(), 1062, "Synthetic native error");
        ...
        for (int attempt = 1; attempt <= 2; attempt++)
        {
            ...
            Console.WriteLine($"WLAN attempt {attempt}: status={result.Status}, interfaces={result.Interfaces.Count}, nativeError={result.NativeErrorCode?.ToString() ?? "none"}");
    ...
    private static bool CheckProxySettingsBoundary()
    {
        ...
        Console.WriteLine($"Proxy settings: success={settings.ReadSucceeded}, mode={settings.Mode}, nativeError={settings.Win32Error?.ToString() ?? "none"}");
        foreach (string? value in new[] { settings.AutoConfigUrl, settings.ManualProxy, settings.BypassList })
        {
    ...
    private static bool CheckNetworkEnvironmentBoundary()
    {
        ...
        Console.WriteLine($"Network environment: adapters={snapshot.Adapters.Count}, active={snapshot.Assessment.ActiveAdapterCount}, wireless={snapshot.Assessment.ActiveWirelessCount}, gateways={snapshot.Assessment.ActiveDefaultGatewayCount}, ambiguous={snapshot.Assessment.RouteSelectionMayBeAmbiguous}");
        ...
        foreach (LocalNetworkAdapterSnapshot adapter in snapshot.Adapters)
        {
            if (string.IsNullOrWhiteSpace(adapter.DisplayName)
                || adapter.DisplayName.Contains('\r') || adapter.DisplayName.Contains('\n'))
            {
                Console.Error.WriteLine("An adapter display name is empty or contains a line break.");
                return false;
            }
            if (adapter.GatewayCount < 0 || adapter.UnicastAddressCount < 0 || adapter.SpeedBitsPerSecond is <= 0)
            {
                Console.Error.WriteLine("An adapter exposed an invalid count or link speed.");
                return false;
            }
            if (!string.IsNullOrWhiteSpace(adapter.InterfaceId) && adapter.InterfaceId.Contains('\n'))
            {
    ...
    {
        WlanInterfaceIdentityReadResult native = WlanInterfaceIdentityReader.ReadCurrent();
        Console.WriteLine($"WLAN identities: success={native.IsSuccess}, interfaces={native.Interfaces.Count}");
        ...
        foreach (WlanInterfaceIdentity identity in native.Interfaces)
        {
            ...
            if (identity.Description.Contains('\r') || identity.Description.Contains('\n'))
            {
        ...
        const string expectedId = "A1B2C3D4-E5F6-47A8-9123-1234567890AB";
        WlanSnapshot snapshot = new(
            Timestamp: DateTimeOffset.UnixEpoch, IsConnected: true,
            Ssid: "SYNTHETIC", Bssid: "00:00:00:00:00:00", RssiDbm: -55,
            Channel: 36, PhyType: "802.11ax", ReceiveLinkSpeedBps: 1_200_000_000,
            TransmitLinkSpeedBps: 1_200_000_000,
            InterfaceDescription: "  Synthetic   Wi-Fi Adapter ", InterfaceId: null);
        WlanInterfaceIdentityReadResult synthetic = new(
            IsSuccess: true,
            Interfaces: [new WlanInterfaceIdentity(expectedId.ToLowerInvariant(), "Synthetic Wi-Fi Adapter", true)],
            Message: "Synthetic identity list");
        WlanSnapshot? attached = WlanInterfaceIdentityReader.AttachIdentity(snapshot, synthetic);
        if (!string.Equals(attached?.InterfaceId, expectedId, StringComparison.OrdinalIgnoreCase))
        {
            Console.Error.WriteLine("A unique connected WLAN identity was not attached to the snapshot.");
            return false;
        }
        WlanSnapshot existing = snapshot with { InterfaceId = "B1B2C3D4-E5F6-47A8-9123-1234567890AB" };
        WlanSnapshot? unchanged = WlanInterfaceIdentityReader.AttachIdentity(existing, synthetic);
        ...
        WlanInterfaceIdentity secondIdentity = new(
            ...
            Description: originalIdentity.Description, IsConnected: originalIdentity.IsConnected);
        WlanInterfaceIdentityReadResult duplicates = synthetic with
        {
            Interfaces = new WlanInterfaceIdentity[] { originalIdentity, secondIdentity }
        };
        WlanSnapshot? duplicateResult = WlanInterfaceIdentityReader.AttachIdentity(snapshot, duplicates);
        ...
        WlanInterfaceIdentity disconnectedIdentity = new(
            InterfaceId: originalIdentity.InterfaceId, Description: originalIdentity.Description, IsConnected: false);
        WlanInterfaceIdentityReadResult disconnected = synthetic with
        {
            Interfaces = new WlanInterfaceIdentity[] { disconnectedIdentity }
        };
        WlanSnapshot? disconnectedResult = WlanInterfaceIdentityReader.AttachIdentity(snapshot, disconnected);
