...
증상 선택, 로컬 IP·게이트웨이·DNS 설정 확인, 승인 대상 파일 가져오기, 고급 옵션 접기, 저장 전 결과 요약을 구현했습니다. 공개 첨부는 포터블 ZIP 하나로 제한합니다. Windows 검증은 PR 체크와 릴리스 워크플로 결과를 기준으로 확인합니다. 세부 범위: [alpha.12 변경 사항](docs/RELEASE_NOTES_0.1.0-alpha.12.md).
...
PR #131의 무선 중심 OSI 화면 구현은 Windows 검증을 통과했다. 검증 소스는 `70be17f5c962499334a69a890a8acb3cb63575da`이며 Windows `34099845395`, Guide `34099845461`, Report `34099845558`, Package `34099845388` 모두 성공했다. 전체 13/13 suite, 중앙 UI 33개 그룹, 패키지 구조·필수 문서 검사가 통과했다. 두 너비의 WPF 렌더링을 확인했으며 글자 누락 의심은 원본 파일·픽셀 비교로 판독 오류였음을 확인했다.

다섯 단계 탐색, 고급 도구, 결과 근거·다음 점검, 유선 비교 안내와 관찰 PHY·시각 표시를 반영했다. 공개 배포와 게시 후 검증은 `docs/WIRELESS_GUIDED_DIAGNOSTICS.md`의 배포 게이트를 따른다. 아래 진행 중 문구와 실패 기록은 이전 revision의 이력이다.

이번 OSI 화면 개편을 제품 전체 완료로 확대하지 않는다. 기존 비동기 WinHTTP(#20), 회사 환경·로밍·절전 검증, 코드서명 및 legacy 보고서 rollback은 별도 범위다.
