...
public partial class MainWindow
{
    ...
    internal void EnsureRepeatedMeasurementTab()
    {
        if (_repeatedMeasurementTabAdded || _applicationOperationWindowClosed)
        {
            return;
        }
    ...
    private TabItem CreateRepeatedMeasurementTab()
    {
        ...
        _repeatCancelButton = new Button
        {
            Content = "반복 측정 중지",
            MinWidth = 125,
            Padding = new Thickness(12, 8, 12, 8),
        ...
        _repeatCancelButton.Click += OnCancelRepeatedMeasurementClick;
        ...
        settingsGrid.ColumnDefinitions.Add(new ColumnDefinition { Width = new GridLength(18) });
        settingsGrid.ColumnDefinitions.Add(new ColumnDefinition());
        settingsGrid.ColumnDefinitions.Add(new ColumnDefinition { Width = new GridLength(18) });
        settingsGrid.ColumnDefinitions.Add(new ColumnDefinition());

        FrameworkElement repeatCountPanel = CreateRepeatSettingPanel(
            "본 측정 횟수", _repeatCountComboBox, "1~5회 · 대표값은 중앙값");
        ...
        buttons.Children.Add(new Border { Width = 12 });
        buttons.Children.Add(_repeatCancelButton);
        ...
        content.Children.Add(new Border
        {
            ...
            {
                TextWrapping = TextWrapping.Wrap,
                Text = "반복 횟수만큼 실제 HEAD/GET 다운로드가 발생합니다. 대상 하나와 작업 전체의 최대 예상 수신량은 각각 2GiB로 제한하며, 외부 URL은 최대 4개를 순차 측정합니다. 중지는 현재 호출이 반환된 뒤 반영되며 확보한 결과는 보존합니다."
            }
        });
    ...
    private static FrameworkElement CreateRepeatSettingPanel(
        string title, Control control, string description)
    {
        StackPanel panel = new();
        panel.Children.Add(new TextBlock { FontWeight = FontWeights.SemiBold, Text = title });
    ...
    private async void OnStartRepeatedInternalClick(object sender, RoutedEventArgs e)
    {
        if (_repeatedOperationLease is not null) return;
        if (!TryCreateRepeatedTargets(NetworkPathKind.Internal,
                out MeasurementTargetDefinition[] targets,
                out RepeatedMeasurementPlan plan, out string error))
        {
    ...
    private async void OnStartRepeatedExternalClick(object sender, RoutedEventArgs e)
    {
        if (_repeatedOperationLease is not null) return;
        if (!TryCreateRepeatedTargets(NetworkPathKind.External,
                out MeasurementTargetDefinition[] targets,
                out RepeatedMeasurementPlan plan, out string error))
        {
            SetRepeatedResult(error, Brushes.DarkRed);
    ...
    private Task RunRepeatedOperationAsync(
        IReadOnlyList<MeasurementTargetDefinition> targets,
        RepeatedMeasurementPlan plan) =>
        RunRepeatedMeasurementSessionAsync(targets, plan, RepeatedMeasurementRunner.RunAsync);
        ...
        out string error)
    {
        ...
        if (_repeatCountComboBox is null || _repeatDelayTextBox is null || _repeatWarmupCheckBox is null)
        {
        ...
        plan = new RepeatedMeasurementPlan(repeatCount,
            _repeatWarmupCheckBox.IsChecked == true, delayMilliseconds);
        ...
        targets = urls.Select((url, index) => new MeasurementTargetDefinition(
            ...
            MaxBytes: maxBytes, TimeoutSeconds: timeoutSeconds,
            Streams: streams, MaxRedirects: maxRedirects)).ToArray();
        ...
        foreach (MeasurementTargetDefinition target in targets)
        {
            ...
            totalPlannedBytes = checked(totalPlannedBytes + plan.GetPlannedMaximumBytes(target));
        }
    ...
    private static string FormatRepeatedResults(
        IReadOnlyList<RepeatedMeasurementResult> results, long plannedBytes)
    {
        if (results.Count == 0) return "완료된 반복 측정 결과가 없습니다.";
        StringBuilder builder = new();
    ...
    private static string FormatRepeatedConfidence(RepeatedMeasurementConfidence confidence) =>
        confidence switch
        {
    ...
    {
        if (_repeatResultText is null || _applicationOperationWindowClosed) return;
    ...
    {
        bool enabled = !_applicationOperationWindowClosed
            && StartInternalMeasurementButton.IsEnabled
            && !_measurementRunning && _observationCancellation is null
            && _repeatedOperationLease is null;
        if (_repeatInternalButton is not null) _repeatInternalButton.IsEnabled = enabled;
        if (_repeatExternalButton is not null) _repeatExternalButton.IsEnabled = enabled;
        if (_repeatCountComboBox is not null) _repeatCountComboBox.IsEnabled = enabled;
        if (_repeatWarmupCheckBox is not null) _repeatWarmupCheckBox.IsEnabled = enabled;
        if (_repeatDelayTextBox is not null) _repeatDelayTextBox.IsEnabled = enabled;
        if (_repeatCancelButton is not null)
        {
            var state = CurrentApplicationOperation;
            _repeatCancelButton.IsEnabled = !_applicationOperationWindowClosed
                && _repeatedOperationLease?.IsCurrent == true
                && state.SupportsCancellation && !state.CancellationRequested
