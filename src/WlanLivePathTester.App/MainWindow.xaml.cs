...
public partial class MainWindow : Window
{
    ...
    {
        await RunBasicDiagnosticAsync(
            ApplicationOperationKind.WlanStatusRead,
            static () =>
            {
                ...
                if (connected is null)
                {
                    ...
                    return $"{result.Message}{Environment.NewLine}{interfaceStates}";
                }
    ...
    {
        await RunBasicDiagnosticAsync(
            ApplicationOperationKind.ProxySettingsRead,
            static () =>
            {
            ...
            text => ProxyResultText.Text = text,
            "현재 사용자 프록시 설정을 확인하고 있습니다.");
    }

    private async void OnResolveProxyRouteClick(object sender, RoutedEventArgs e)
    {
        using ApplicationOperationUiLease? lease = TryBeginUiApplicationOperation(ApplicationOperationKind.ProxyRouteResolution);
        if (lease is null)
        {
        ...
        NetworkPathKind expectedPath = ProxyExpectedPathComboBox.SelectedIndex == 1 ? NetworkPathKind.Internal : NetworkPathKind.External;
        ...
        try
        {
            ProxyRouteResolution result = await Task.Run(() => ProxyRouteResolver.Resolve(url, expectedPath));
            ...
            if (result.InvalidDirectiveCount > 0) builder.AppendLine($"제외한 지시문: {result.InvalidDirectiveCount}개");
            if (result.Win32ErrorCode is int errorCode) builder.AppendLine($"Win32 오류 코드: {errorCode}");
        ...
        catch (Exception exception)
        {
            ...
            ProxyRouteResultText.Text = $"프록시 경로 확인 오류: {exception.GetType().Name}. 예외 원문은 표시하지 않았습니다.";
        }
        finally
        {
    ...
    {
        if (!TryReadMeasurementSettings(out long maxBytes, out int timeoutSeconds,
                out int streams, out int maxRedirects, out string settingsError))
        {
        ...
        if (!NetworkInputBoundary.TryHttpUri(InternalTargetUrlTextBox.Text,
                out Uri? normalizedUri, out string urlError))
        {
            ShowMeasurementInputError(InternalMeasurementResultText, urlError);
            return;
        }
        MeasurementTargetDefinition target = new(
            Name: "내부망 기준 대상", Url: normalizedUri.OriginalString,
            PathKind: NetworkPathKind.Internal, RequireProxy: false, RequireDirect: true,
            MaxBytes: maxBytes, TimeoutSeconds: timeoutSeconds, Streams: streams, MaxRedirects: maxRedirects);
        await RunMeasurementOperationAsync(async cancellationToken =>
        {
            ...
            DownloadMeasurementResult result = await DownloadMeasurementRunner.RunAsync(
                target, MeasurementHeadPreflightCheckBox.IsChecked == true, cancellationToken);
    ...
    private async void OnStartExternalMeasurementClick(object sender, RoutedEventArgs e)
    {
        ...
        if (!NetworkInputBoundary.TryHttpUrlLines(ExternalTargetUrlsTextBox.Text,
                out string[] urls, out string listError))
        {
            ShowMeasurementInputError(ExternalMeasurementResultText, listError);
            return;
        }
        MeasurementTargetDefinition[] targets = urls.Select((url, index) => new MeasurementTargetDefinition(
            Name: $"외부 대상 {index + 1}", Url: url, PathKind: NetworkPathKind.External,
            RequireProxy: true, RequireDirect: false, MaxBytes: maxBytes,
            TimeoutSeconds: timeoutSeconds, Streams: streams, MaxRedirects: maxRedirects)).ToArray();
        await RunMeasurementOperationAsync(async cancellationToken =>
        {
            ...
            IReadOnlyList<DownloadMeasurementResult> results = await DownloadMeasurementRunner.RunManyAsync(
                targets, MeasurementHeadPreflightCheckBox.IsChecked == true, cancellationToken);
            ExternalMeasurementResultText.Foreground = results.All(result => result.IsSuccess)
                ? Brushes.DarkGreen : results.Any(result => result.IsSuccess) ? Brushes.DarkOrange : Brushes.DarkRed;
    ...
    {
        if (!_measurementRunning || _cancelMeasurement is null) return;
    ...
    private bool TryReadMeasurementSettings(out long maxBytes, out int timeoutSeconds,
        out int streams, out int maxRedirects, out string error)
    {
        maxBytes = 0; timeoutSeconds = 0; streams = MeasurementStreamsComboBox.SelectedIndex + 1;
        maxRedirects = 0; error = string.Empty;
        if (!int.TryParse(MeasurementMaxMegabytesTextBox.Text.Trim(), out int maxMegabytes)
            || maxMegabytes is < 1 or > 1024)
        { error = "최대 수신량은 1~1024MB 범위의 정수여야 합니다."; return false; }
        if (!int.TryParse(MeasurementTimeoutSecondsTextBox.Text.Trim(), out timeoutSeconds)
            || timeoutSeconds is < 5 or > 300)
        { error = "제한 시간은 5~300초 범위의 정수여야 합니다."; return false; }
        if (streams is < 1 or > 4)
        { error = "스트림 수는 1~4 범위여야 합니다."; return false; }
        if (!int.TryParse(MeasurementMaxRedirectsTextBox.Text.Trim(), out maxRedirects)
            || maxRedirects is < 0 or > 10)
        { error = "최대 리다이렉트 수는 0~10 범위의 정수여야 합니다."; return false; }
    ...
    { target.Foreground = Brushes.DarkRed; target.Text = $"입력 오류: {message}"; }
    ...
    private static string FormatMeasurementResults(IReadOnlyList<DownloadMeasurementResult> results)
    {
        ...
        {
            if (index > 0) { builder.AppendLine(); builder.AppendLine(new string('-', 52)); }
    ...
    {
        List<string> values = [];
        AddHeader(values, headers, "Age"); AddHeader(values, headers, "Cache-Status");
        AddHeader(values, headers, "X-Cache"); AddHeader(values, headers, "Content-Length");
    ...
    {
        MeasurementStatus.NotRun => "미실행", MeasurementStatus.Success => "성공",
        MeasurementStatus.PartialSuccess => "부분 성공", MeasurementStatus.Failed => "실패",
        MeasurementStatus.TimedOut => "시간 초과", MeasurementStatus.Canceled => "취소",
        MeasurementStatus.Blocked => "정책 차단", MeasurementStatus.ProxyAuthenticationRequired => "프록시 인증 실패",
        MeasurementStatus.PathMismatch => "기대 경로 불일치", _ => status.ToString()
    };
    private static string FormatProxyUsage(bool? proxyWasUsed) => proxyWasUsed switch { true => "사용", false => "미사용", null => "확인 불가" };
    ...
    {
        ProxyResolutionStatus.Success => "성공", ProxyResolutionStatus.InvalidUrl => "URL 오류",
        ...
        ProxyResolutionStatus.TimedOut => "시간 초과", _ => "Windows API 오류"
    };
    private static string FormatProxySource(ProxyConfigurationSource source) => source switch
    {
        ProxyConfigurationSource.None => "설정 없음", ProxyConfigurationSource.Manual => "수동 프록시 또는 바이패스",
        ProxyConfigurationSource.Wpad => "WPAD 자동 검색", ProxyConfigurationSource.Pac => "명시적 PAC",
        ProxyConfigurationSource.WpadThenPac => "WPAD 실패 후 명시적 PAC",
        ProxyConfigurationSource.ManualFallback => "PAC/WPAD 실패 후 수동 설정", _ => "확인 불가"
    };
    ...
    private static string FormatExpectedPath(NetworkPathKind pathKind) =>
        pathKind == NetworkPathKind.Internal ? "내부망 — DIRECT 예상" : "외부망 — PROXY 예상";
    ...
    private static string FormatLinkSpeed(ulong? value) => value is ulong bitsPerSecond
        ? $"{bitsPerSecond / 1_000_000d:F1} Mbps" : "확인 불가";
}
