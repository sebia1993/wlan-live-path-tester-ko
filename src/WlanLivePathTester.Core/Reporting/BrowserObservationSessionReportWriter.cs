...
public static class BrowserObservationSessionReportWriter
{
        ...
        DateTimeOffset? generatedAt = null)
    {
        ...
        return new BrowserObservationSessionReportDocument(
            ...
            DataHandlingStatement: "브라우저 관찰 보고서는 현재 PC에서 생성되며 자동 업로드, 텔레메트리 또는 온라인 분석을 수행하지 않습니다.",
            ...
            Limitations:
            [
                "관찰값은 브라우저 프로세스 한 개가 아니라 고정된 물리 Wi-Fi 인터페이스 전체 수신·송신 카운터입니다.",
                "다른 프로그램의 통신이 포함될 수 있으므로 관찰 신뢰도는 최대 Medium입니다.",
                "같은 물리 Wi-Fi에서 BSSID가 바뀌는 로밍은 인터페이스 변경과 구분합니다.",
                "SSID, BSSID, 인터페이스 ID, 인터페이스 이름, IP, MAC, 게이트웨이와 다운로드 URL은 보고서에 포함하지 않습니다.",
                "외부 다운로드 결과는 회사 프록시, 보안 정책과 외부 사이트 또는 CDN의 영향을 받을 수 있습니다."
            ]);
    }
    ...
    public static BrowserObservationSessionReportExportResult WriteAll(
        ...
        string outputDirectory,
        string filePrefix = "WlanBrowserObservation")
    {
        ...
        BrowserObservationSessionReportDocument report)
    {
        ...
        if (report.Summary is BrowserObservationSessionReportSummary summary)
        {
            ...
            foreach (BrowserObservationSessionReportSample sample in summary.Samples)
            {
        ...
        BrowserObservationSessionReportDocument report)
    {
        ...
        builder.Append("<title>브라우저 다운로드 관찰 보고서</title><style>");
        ...
        builder.Append("<header><h1>브라우저 다운로드 관찰 보고서</h1><div class=\"sub\">");
        Html(
            builder,
        ...
        builder.Append("</p><p class=\"small\">SSID·BSSID·인터페이스 ID·IP·MAC·게이트웨이·URL은 포함하지 않습니다. 이 HTML은 외부 리소스와 스크립트를 사용하지 않습니다.</p></section>");

        builder.Append("<section class=\"card\"><h2>종료 상태</h2><p><span class=\"badge ");
        ...
        {
            builder.Append("<section class=\"card\"><h2>관찰 요약</h2><p>저장된 관찰 샘플이 없습니다.</p></section>");
        }
    ...
    private static BrowserObservationSessionReportSummary MapSummary(
        ...
        new(
            ...
            ObservedSeconds: Math.Max(0, summary.ObservedDuration.TotalSeconds),
            ...
            SummaryMessage: Redact(summary.Message),
            Limitation: Redact(summary.Limitation),
            Samples: summary.Samples
                .Select((sample, index) => MapSample(sample, index + 1))
                .ToArray());
    ...
    private static BrowserObservationSessionReportSample MapSample(
        ...
        new(
            ...
            IntervalSeconds: Math.Max(0, sample.Interval.TotalSeconds),
        ...
        BrowserObservationSessionReportSummary summary)
    {
        ...
        AddCsv(builder, "summary", "observedSeconds", Invariant(summary.ObservedSeconds));
        AddCsv(builder, "summary", "baselineReceiveMbps", Invariant(summary.BaselineReceiveMbps));
        AddCsv(builder, "summary", "averageAdjustedReceiveMbps", Invariant(summary.AverageAdjustedReceiveMbps));
        AddCsv(builder, "summary", "peakAdjustedReceiveMbps", Invariant(summary.PeakAdjustedReceiveMbps));
        AddCsv(builder, "summary", "totalReceiveBytes", Invariant(summary.TotalReceiveBytes));
        AddCsv(builder, "summary", "activeSampleCount", Invariant(summary.ActiveSampleCount));
        AddCsv(builder, "summary", "pauseCount", Invariant(summary.PauseCount));
        AddCsv(builder, "summary", "suddenDropCount", Invariant(summary.SuddenDropCount));
        AddCsv(builder, "summary", "bssidChangeCount", Invariant(summary.BssidChangeCount));
        AddCsv(builder, "summary", "adapterChangeCount", Invariant(summary.AdapterChangeCount));
        AddCsv(builder, "summary", "counterResetCount", Invariant(summary.CounterResetCount));
        AddCsv(builder, "summary", "wlanDisconnectedSampleCount", Invariant(summary.WlanDisconnectedSampleCount));
        ...
        BrowserObservationSessionReportSample sample)
    {
        ...
        AddCsv(builder, section, "intervalSeconds", Invariant(sample.IntervalSeconds));
        AddCsv(builder, section, "isBaseline", Invariant(sample.IsBaseline));
        AddCsv(builder, section, "receiveBytesDelta", Invariant(sample.ReceiveBytesDelta));
        AddCsv(builder, section, "transmitBytesDelta", Invariant(sample.TransmitBytesDelta));
        AddCsv(builder, section, "rawReceiveMbps", Invariant(sample.RawReceiveMbps));
        AddCsv(builder, section, "rawTransmitMbps", Invariant(sample.RawTransmitMbps));
        AddCsv(builder, section, "adjustedReceiveMbps", Invariant(sample.AdjustedReceiveMbps));
        AddCsv(builder, section, "rssiDbm", Invariant(sample.RssiDbm));
        AddCsv(builder, section, "receiveLinkMbps", Invariant(sample.ReceiveLinkMbps));
        AddCsv(builder, section, "transmitLinkMbps", Invariant(sample.TransmitLinkMbps));
        AddCsv(builder, section, "invalidInterval", Invariant(sample.InvalidInterval));
        AddCsv(builder, section, "adapterChanged", Invariant(sample.AdapterChanged));
        AddCsv(builder, section, "counterReset", Invariant(sample.CounterReset));
        AddCsv(builder, section, "wlanDisconnected", Invariant(sample.WlanDisconnected));
        AddCsv(builder, section, "bssidChanged", Invariant(sample.BssidChanged));
        AddCsv(builder, section, "pauseDetected", Invariant(sample.PauseDetected));
        AddCsv(builder, section, "suddenDropDetected", Invariant(sample.SuddenDropDetected));
    ...
    private static void AppendSummaryHtml(
        ...
        BrowserObservationSessionReportSummary summary)
    {
        builder.Append("<section class=\"card\"><h2>관찰 요약</h2><div class=\"grid\">");
        Metric(builder, "관찰 시간", $"{summary.ObservedSeconds:F1}초");
        Metric(builder, "기준 수신", Mbps(summary.BaselineReceiveMbps));
        Metric(builder, "조정 평균", Mbps(summary.AverageAdjustedReceiveMbps));
        Metric(builder, "조정 최고", Mbps(summary.PeakAdjustedReceiveMbps));
        Metric(builder, "총 수신량", Bytes(summary.TotalReceiveBytes));
        Metric(builder, "활성 샘플", summary.ActiveSampleCount.ToString(CultureInfo.InvariantCulture));
        Metric(builder, "BSSID 변경", summary.BssidChangeCount.ToString(CultureInfo.InvariantCulture));
        Metric(builder, "NIC 변경", summary.AdapterChangeCount.ToString(CultureInfo.InvariantCulture));
        Metric(builder, "카운터 재설정", summary.CounterResetCount.ToString(CultureInfo.InvariantCulture));
        Metric(builder, "WLAN 미확인", summary.WlanDisconnectedSampleCount.ToString(CultureInfo.InvariantCulture));
        Metric(builder, "정지 / 급락", $"{summary.PauseCount} / {summary.SuddenDropCount}");
    ...
    private static void AppendSamplesHtml(
        ...
        IReadOnlyList<BrowserObservationSessionReportSample> samples)
    {
        builder.Append("<section class=\"card\"><h2>시간축 샘플</h2><div class=\"scroll\"><table><thead><tr><th>#</th><th>시각</th><th>구분</th><th>조정 Rx</th><th>원시 Rx</th><th>Tx</th><th>RSSI</th><th>PHY Rx/Tx</th><th>상태</th></tr></thead><tbody>");
        foreach (BrowserObservationSessionReportSample sample in samples)
        {
            builder.Append("<tr><td>");
            Html(builder, sample.Sequence.ToString(CultureInfo.InvariantCulture));
            builder.Append("</td><td class=\"nowrap\">");
            Html(builder, sample.Timestamp.ToLocalTime().ToString("HH:mm:ss.fff", CultureInfo.InvariantCulture));
            ...
            builder.Append("</td><td>");
            Html(builder, sample.RssiDbm?.ToString(CultureInfo.InvariantCulture) ?? "-");
            builder.Append("</td><td class=\"nowrap\">");
            Html(builder, $"{Mbps(sample.ReceiveLinkMbps)} / {Mbps(sample.TransmitLinkMbps)}");
            builder.Append("</td><td>");
    ...
    {
        List<string> flags = [];
        if (sample.InvalidInterval) flags.Add("간격오류");
        if (sample.AdapterChanged) flags.Add("NIC변경");
        if (sample.CounterReset) flags.Add("카운터재설정");
        if (sample.WlanDisconnected) flags.Add("WLAN미확인");
        if (sample.BssidChanged) flags.Add("BSSID변경");
        if (sample.PauseDetected) flags.Add("정지");
        if (sample.SuddenDropDetected) flags.Add("급락");
        if (!string.IsNullOrWhiteSpace(sample.Note)) flags.Add(sample.Note);
        return flags.Count == 0 ? "정상" : string.Join(" · ", flags);
    }

    private static string TerminationCss(string reason) =>
        reason.Equals("Completed", StringComparison.OrdinalIgnoreCase)
            ? "ok"
            : reason.Equals("CanceledByUser", StringComparison.OrdinalIgnoreCase)
                ? "warn"
                : "bad";
    ...
    private static string Mbps(double value) => $"{value:F1} Mbps";

    private static string Mbps(double? value) =>
        value.HasValue ? Mbps(value.Value) : "계산 안 함";
