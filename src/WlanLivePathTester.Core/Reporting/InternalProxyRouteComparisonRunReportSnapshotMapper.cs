    ...
    InternalProxyRouteComparisonRunReportSnapshotMapper
{
    ...
    public static InternalProxyRouteComparisonRunReportSnapshot
        FromResult(InternalProxyRouteComparisonRunResult result)
    {
        ...
        ProxyEndpointRouteAnalysisResult? analysis =
            result.ProxyExecution?.Analysis;
        ...
        return new InternalProxyRouteComparisonRunReportSnapshot(
            ...
            ProxySelectionStatus:
                SafeEnum(result.ProxySelectionStatus),
            ProxyPlanStatus: SafeEnum(result.ProxyPlanStatus),
            ProxyPlanCode: SafeEnum(result.ProxyPlanCode),
            ProxyExecutionStatus:
                SafeNullableEnum(result.ProxyExecutionStatus),
            ProxyEndpointSourceKind:
                SafeEnum(result.ProxyEndpointSourceKind),
            ProxyDecision: SafeEnum(result.ProxyDecision),
            TargetScheme: SafeTargetScheme(result.TargetScheme),
            InternalRouteStatus:
                SafeNullableEnum(result.InternalRouteStatus),
            ProxyRouteStatus:
                SafeNullableEnum(result.ProxyRouteStatus),
            ParsedProxyEndpointCount:
                Count(result.ParsedProxyEndpointCount),
            ApplicableProxyEndpointCount:
                Count(result.ApplicableProxyEndpointCount),
            AnalyzedProxyEndpointCount:
                Count(result.AnalyzedProxyEndpointCount),
            SuccessfulProxyEndpointCount:
                Count(result.SuccessfulProxyEndpointCount),
            DistinctProxyInterfaceCount:
                Count(result.DistinctProxyInterfaceCount),
            ...
            ProxyParseErrorsPresent:
                result.ProxyParseErrorsPresent,
            ...
            ProxyEntries: MapProxyEntries(analysis),
            Finding: MapFinding(finding));
    }

    private static InternalProxyRouteComparisonReportComparison?
        MapComparison(
            InternalProxyRouteComparisonResult? comparison)
    {
        ...
        return new InternalProxyRouteComparisonReportComparison(
            ...
            InternalRouteStatus:
                SafeNullableEnum(comparison.InternalRouteStatus),
            ProxyExecutionStatus:
                SafeNullableEnum(comparison.ProxyExecutionStatus),
            ProxyAnalysisStatus:
                SafeNullableEnum(comparison.ProxyAnalysisStatus),
            ProxySourceKind:
                SafeNullableEnum(comparison.ProxySourceKind),
            ProxyPlanCode:
                SafeNullableEnum(comparison.ProxyPlanCode),
            ...
            ProxyApplicableEndpointCount:
                Count(comparison.ProxyApplicableEndpointCount),
            ProxyAnalyzedEndpointCount:
                Count(comparison.ProxyAnalyzedEndpointCount),
            ProxySuccessfulEndpointCount:
                Count(comparison.ProxySuccessfulEndpointCount),
            ProxyDistinctInterfaceCount:
                Count(comparison.ProxyDistinctInterfaceCount),
            ProxySkippedAfterDirectCount:
                Count(comparison.ProxySkippedAfterDirectCount),
            ProxyDirectPresent: comparison.ProxyDirectPresent,
            ProxyDirectIsPrimary:
                comparison.ProxyDirectIsPrimary,
            ProxyDirectFallbackPresent:
                comparison.ProxyDirectFallbackPresent,
            ProxyParseErrorsPresent:
                comparison.ProxyParseErrorsPresent,
    ...
    private static IReadOnlyList<
        InternalProxyRouteComparisonReportProxyEntry> MapProxyEntries(
            ProxyEndpointRouteAnalysisResult? analysis)
    {
        if (analysis is null || analysis.Endpoints.Count == 0)
        {
            return Array.Empty<
                InternalProxyRouteComparisonReportProxyEntry>();
        }

        return analysis.Endpoints
            .OrderBy(endpoint => Math.Max(0, endpoint.Sequence))
            .Select(endpoint =>
                new InternalProxyRouteComparisonReportProxyEntry(
                    Sequence: Math.Max(0, endpoint.Sequence),
                    AppliesToScheme:
                        SafeEndpointScheme(
                            endpoint.AppliesToScheme),
                    Transport: SafeEnum(endpoint.Transport),
                    Port: endpoint.Port is >= 1 and <= 65535
                        ? endpoint.Port
                        : null,
                    HostFingerprint:
                        SafeFingerprint(endpoint.HostFingerprint),
                    RouteStatus: SafeEnum(endpoint.RouteStatus),
                    WlanCorrelationStatus:
                        SafeEnum(endpoint.WlanCorrelationStatus),
                    ...
                    ResolvedAddressCount:
                        Count(endpoint.ResolvedAddressCount),
                    SuccessfulAddressCount:
                        Count(endpoint.SuccessfulAddressCount),
                    FailedAddressCount:
                        Count(endpoint.FailedAddressCount)))
            .ToArray();
    }
    ...
    private static InternalProxyRouteComparisonReportFinding MapFinding(
        ...
        new(
            ...
            Title: SafeNarrative(finding.Title),
            Evidence: SafeNarrative(finding.Evidence),
            Interpretation: SafeNarrative(
                finding.Interpretation),
            Limitation: SafeNarrative(finding.Limitation),
            NextStep: SafeNarrative(finding.NextStep));
    ...
    private static string SafeNullableEnum<TEnum>(
        TEnum? value,
        bool returnNullWhenMissing = false)
        where TEnum : struct, Enum
    {
        if (!value.HasValue)
        {
            return returnNullWhenMissing
                ? string.Empty
                : "None";
        }
    ...
    private static string SafeSeverity(string? value) =>
