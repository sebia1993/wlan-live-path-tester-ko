...
public sealed class InternalProxyRouteComparisonCoordinator
{
    ...
    private readonly IInternalDirectRouteEvidenceReader _internalRouteReader;
    private readonly ProxyDirectiveRouteAnalysisCoordinator _proxyCoordinator;
        ...
        TimeProvider? timeProvider = null)
    {
        _internalRouteReader = internalRouteReader
            ?? throw new ArgumentNullException(nameof(internalRouteReader));
        _proxyCoordinator = proxyCoordinator
            ?? throw new ArgumentNullException(nameof(proxyCoordinator));
        _timeProvider = timeProvider ?? TimeProvider.System;
    }

    public Task<InternalProxyRouteComparisonRunResult> RunManualDirectiveAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        string rawInternalTarget = internalTarget ?? string.Empty;
        bool expectedWlanIdentityAvailable = HasValidInterfaceGuid(expectedWlanInterfaceId);
        ProxyDirectiveRouteAnalysisPlan plan = ProxyDirectiveRouteAnalysisPlanPolicy.Create(selection);

        string? inputError = ValidateInput(rawInternalTarget, externalTarget);
        ...
        switch (plan.Status)
        {
            ...
            case ProxyDirectiveRouteAnalysisPlanStatus.AnalyzeProxyEndpoints:
                break;
            default:
                return CreateResult(
                    InternalProxyRouteComparisonRunStatus.ProxySourceBlocked,
        ...
        ProxyEndpointParseResult parsed = ProxyEndpointParser.Parse(directiveText, externalTarget);
        if (!parsed.IsUsable || parsed.Errors.Count > 0
            || parsed.Decision == ProxyEndpointDecision.Unknown)
        {
            return CreateResult(
                InternalProxyRouteComparisonRunStatus.InvalidInput,
                plan, parsed, null, null, null,
                expectedWlanIdentityAvailable,
                false, false,
        ...
        if (cancellationToken.IsCancellationRequested
            || internalRoute.Status == DestinationRouteEvidenceStatus.Canceled)
        {
            return CreateCanceled(
                plan, parsed, null, internalRoute, null,
                expectedWlanIdentityAvailable,
                true, false, externalTarget);
        }
        ...
        if (!CanContinueWithInternalRoute(internalRoute))
        {
            InternalProxyRouteComparisonResult? terminalComparison =
                internalRoute.Status == DestinationRouteEvidenceStatus.MultipleInterfaces
                    ? InternalProxyRouteComparisonEvaluator.Evaluate(
                        internalRoute, proxyExecution: null, _timeProvider.GetUtcNow())
                    : null;
            return CreateResult(
                InternalProxyRouteComparisonRunStatus.InternalRouteUnavailable,
                plan, parsed, null, terminalComparison, internalRoute,
                expectedWlanIdentityAvailable,
                true, false,
        ...
        ProxyDirectiveRouteAnalysisExecutionResult<ProxyEndpointRouteAnalysisResult> execution;
        try
        {
        ...
        catch (OperationCanceledException)
        {
            return CreateCanceled(
                plan, parsed, null, internalRoute, null,
                expectedWlanIdentityAvailable,
                true, true, externalTarget);
        }
        catch (Exception)
        {
            return CreateResult(
                InternalProxyRouteComparisonRunStatus.Failed,
                plan, parsed, null, null, internalRoute,
                expectedWlanIdentityAvailable,
                true, true,
        ...
        {
            return CreateCanceled(
                plan, parsed, execution, internalRoute, null,
                expectedWlanIdentityAvailable,
                true, true, externalTarget);
        }
        ...
        InternalProxyRouteComparisonRunStatus runStatus = execution.Status switch
        {
    ...
    private static string? ValidateInput(string internalTarget, Uri? externalTarget)
    {
        if (!NetworkInputBoundary.TryRouteHost(internalTarget, out _, out _))
        {
            return $"내부 DIRECT 기준 대상은 제어 문자 없이 {MaximumInternalTargetLength}자 이하의 URL, 호스트 이름 또는 IP 주소여야 합니다.";
        }
        if (!NetworkInputBoundary.IsHttpUri(externalTarget, out _))
        {
            return "외부 대상은 사용자 정보·fragment·제어 문자 없는 절대 HTTP 또는 HTTPS URL이어야 합니다.";
        }
        return null;
    }

    private static bool CanContinueWithInternalRoute(DestinationRouteEvidence evidence) =>
        evidence.Status == DestinationRouteEvidenceStatus.Success
        && evidence.SelectedInterface is not null
        && HasValidInterfaceGuid(evidence.SelectedInterface.InterfaceIdentity);

    private static bool HasValidInterfaceGuid(string? value) =>
        Guid.TryParse((value ?? string.Empty).Trim().Trim('{', '}'), out _);
    ...
    {
        ProxyEndpointRouteAnalysisResult? analysis = execution?.Analysis;
        bool directPresent = forceDirectPrimary
            || parsed?.DirectPresent == true
            || analysis?.DirectPresent == true
            || plan.DirectiveText?.Trim().Equals("DIRECT", StringComparison.OrdinalIgnoreCase) == true;
        ...
        bool directFallback = !directIsPrimary
            && (analysis?.DirectFallback ?? parsed?.DirectFallback ?? false);
        ...
        return new InternalProxyRouteComparisonRunResult(
            ...
            ProxyEndpointSourceKind:
                parsed?.SourceKind ?? analysis?.SourceKind ?? ProxyEndpointSourceKind.Unknown,
            ProxyDecision:
                analysis?.ProxyDecision ?? parsed?.Decision
                ?? (directIsPrimary ? ProxyEndpointDecision.Direct : ProxyEndpointDecision.Unknown),
            TargetScheme:
                analysis?.TargetScheme ?? parsed?.TargetScheme ?? externalTarget?.Scheme.ToLowerInvariant(),
            InternalRouteStatus: internalRoute?.Status,
            ProxyRouteStatus: analysis?.Status,
            Comparison: comparison,
            ParsedProxyEndpointCount: Math.Max(
                0, parsed?.ParsedEndpointCount ?? analysis?.ParsedEndpointCount ?? 0),
            ApplicableProxyEndpointCount: Math.Max(
                0, analysis?.ApplicableEndpointCount ?? parsed?.Endpoints.Count ?? 0),
            AnalyzedProxyEndpointCount: Math.Max(0, analysis?.AnalyzedEndpointCount ?? 0),
            SuccessfulProxyEndpointCount: Math.Max(0, analysis?.SuccessfulEndpointCount ?? 0),
            DistinctProxyInterfaceCount: Math.Max(0, analysis?.DistinctInterfaceCount ?? 0),
            DirectPresent: directPresent,
            DirectIsPrimary: directIsPrimary,
            DirectFallback: directFallback,
            ProxyParseErrorsPresent: plan.HasParseErrors || parsed?.Errors.Count > 0,
            ExpectedWlanIdentityAvailable: expectedWlanIdentityAvailable,
            InternalRouteReadPerformed: internalRouteReadPerformed,
            ProxyRouteAnalysisPerformed: proxyRouteAnalysisPerformed,
            Message: message,
