...
{
    Ready, Direct, NeedsAutomaticLookupConsent, NoConfiguredProxy, InvalidInput,
    ConfigurationReadFailed, AutomaticResolutionFailed, UnsafeOrUnsupportedResult,
    Canceled, Busy, Failed
}

// Original URLs and the selection stay private, in memory only.
public sealed class WindowsRouteProxyImportResult
{
    ...
    internal WindowsRouteProxyImportResult(
        WindowsRouteProxyImportStatus status, ProxyConfigurationSource source,
        Uri? target, ProxyDirectiveSourceSelectionResult? selection,
        bool automaticLookupAttempted, bool autoLogonRetried, bool wasBypassed,
        TimeProvider clock)
    {
        Status = status;
        Source = source;
    ...
    public string Message => Status switch
    {
        ...
        WindowsRouteProxyImportStatus.InvalidInput => "2048자 이하의 유효한 절대 HTTP(S) 원문 URL과 1000~30000ms 제한 시간이 필요합니다.",
        ...
        WindowsRouteProxyImportStatus.UnsafeOrUnsupportedResult => "설정 원문 또는 판정을 안전하게 해석하지 못했습니다. 일부 후보나 DIRECT로 대체하지 않았습니다.",
    ...
    public bool TryGetSelection(Uri? target,
        [NotNullWhen(true)] out ProxyDirectiveSourceSelectionResult? selection)
    {
        ...
        if (_selection is null || !WindowsRouteProxyImporter.IsValidTarget(target)
            || !string.Equals(_targetKey, target!.AbsoluteUri, StringComparison.Ordinal)) return false;
        TimeSpan age = _clock.GetElapsedTime(_timestamp);
        if (age < TimeSpan.Zero || age >= TimeSpan.FromMinutes(5)) return false;
...
public sealed class WindowsRouteProxyImporter
{
    ...
    public WindowsRouteProxyImporter()
        : this(CurrentUserProxySettingsReader.ReadRaw,
    ...
    {
        _readConfiguration = readConfiguration ?? throw new ArgumentNullException(nameof(readConfiguration));
    ...
    public async Task<WindowsRouteProxyImportResult> ImportAsync(
        Uri? target, bool allowAutomaticProxyLookup = false,
        int timeoutMilliseconds = 5000, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            // Do not abandon a synchronous native call before it releases its resources.
            return await Task.Run(() => Capture(target!, allowAutomaticProxyLookup,
                timeoutMilliseconds, cancellationToken), CancellationToken.None).ConfigureAwait(false);
        }
        finally { Volatile.Write(ref _running, 0); }
    }

    internal static bool IsValidTarget(Uri? target) =>
        NetworkInputBoundary.IsHttpUri(target, out _)
        // Preserve this adapter's established strict identity contract.
        ...
        Uri target, bool allowAutomatic, int timeout, CancellationToken token)
    {
        ...
        try
        {
            ...
            bool pacConfigured = !string.IsNullOrEmpty(configuration.AutoConfigUrl);
            if (pacConfigured && !NetworkInputBoundary.TryHttpUri(configuration.AutoConfigUrl, out _, out _))
                return Result(WindowsRouteProxyImportStatus.UnsafeOrUnsupportedResult);
            bool automatic = configuration.AutoDetectEnabled || pacConfigured;
            if (!automatic) return ImportManual(target, configuration);
            if (!allowAutomatic)
            ...
            foreach (ProxyRouteHop hop in route.Hops)
            {
                if (hop.Kind == ProxyRouteKind.Direct && hop.ProxyUri is null)
                    ...
                    && NetworkInputBoundary.TryHttpUri(hop.ProxyUri, out Uri? proxy, out _)
                    && proxy.Query.Length == 0 && proxy.AbsolutePath == "/")
            ...
            ProxyDirectiveSourceSnapshot snapshot = new(
                ...
                configuration.AutoDetectEnabled, pacConfigured);
    ...
    private WindowsRouteProxyImportResult ImportManual(Uri target, CurrentUserProxyConfiguration configuration)
    {
        if (!NetworkInputBoundary.TrySingleLine(configuration.ManualProxy,
                ProxyEndpointParser.MaximumInputLength, out string manualText, out _, allowEmpty: true)
            || !ProxyBypassMatcher.IsValidList(configuration.BypassList))
            return Result(WindowsRouteProxyImportStatus.UnsafeOrUnsupportedResult);
        ...
        // Never discard a rejected bypass/list decision and then re-select the
        // same input as though its configuration had been accepted.
        if (manual.RouteKind == ProxyRouteKind.Unknown || manual.InvalidDirectiveCount != 0)
            return Result(WindowsRouteProxyImportStatus.UnsafeOrUnsupportedResult);
        ProxyDirectiveSourceSnapshot snapshot = new(
            _clock.GetUtcNow(), ProxyDirectiveSourceReadStatus.NotAttempted,
            false, null, ProxyDirectiveSourceReadStatus.Success, true,
            manual.WasBypassed ? "DIRECT" : configuration.ManualProxy, false, false);
        return FromSnapshot(target, snapshot, ProxyConfigurationSource.Manual,
            attempted: false, autoLogon: false, manual.WasBypassed);
    }

    private WindowsRouteProxyImportResult FromSnapshot(Uri target, ProxyDirectiveSourceSnapshot snapshot,
        ProxyConfigurationSource source, bool attempted, bool autoLogon, bool wasBypassed)
    {
        ProxyDirectiveSourceSelectionResult selection = ProxyDirectiveSourceSnapshotSelectionPolicy.Select(snapshot);
        ...
        if (!selection.HasUsableSelection || !parsed.IsUsable || parsed.Errors.Count > 0
            || parsed.RejectedTokenCount > 0 || parsed.TruncatedTokenCount > 0
            || selection.ParseResult?.Issues.Any(issue => issue.Severity == ProxyDirectiveIssueSeverity.Error) == true)
            return Result(WindowsRouteProxyImportStatus.UnsafeOrUnsupportedResult,
                attempted: attempted, autoLogon: autoLogon);

        bool direct = parsed.Decision is ProxyEndpointDecision.Direct or ProxyEndpointDecision.DirectWithProxyAlternatives;
        return Result(direct ? WindowsRouteProxyImportStatus.Direct : WindowsRouteProxyImportStatus.Ready,
            source, target, selection, attempted, autoLogon, wasBypassed);
    }

    private WindowsRouteProxyImportResult Result(WindowsRouteProxyImportStatus status,
